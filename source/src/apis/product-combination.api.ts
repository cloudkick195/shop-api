...
import { Delete, Patch } from "../decorators/http-method.decorator";
...
export class ProductCombinationApi {
    ...
    @Patch('/:combinationId', [AdminAuthenticationMiddleware])
    public async updateCombination(request: Request, response: Response) {
        try {
            const params: any = request.params;
            if (params.combinationId && params.combinationId > 0) {
                await this.productCombinationRepository.removeCombination(params.combinationId);
